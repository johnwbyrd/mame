...
#define ZBC_LOG(...)     do { if (ZBC_VERBOSE & ZBC_LOG_GENERAL) osd_printf_verbose(__VA_ARGS__); } while (0)
#define ZBC_LOG_MEM(...) do { if (ZBC_VERBOSE & ZBC_LOG_MEMORY) osd_printf_verbose(__VA_ARGS__); } while (0)
...
void zbc_state<CPU_TYPE, LOAD_ADDR, CPU_SPEED, VRAM_ADDR>::machine_start() {
	...
	ZBC_LOG_MEM("  Total RAM: 0x%llX bytes\n\n",
...
zbc_state<CPU_TYPE, LOAD_ADDR, CPU_SPEED, VRAM_ADDR>::quickload_cb(
	...
	ZBC_INFO("ZBC quickload: loaded %llu bytes at 0x%llX, setting PC to 0x%llX\n",
